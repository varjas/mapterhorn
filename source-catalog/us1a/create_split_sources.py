...
import shutil
from grid_utils import deduplicate_files, get_friendly_location, generate_directory_suffix, sort_lat_lon_bands
...
Examples:
  ...
  python create_nested_sources.py --dry-run --lon 3 --lat 1
    ...
    unique_lon_bands = sort_lat_lon_bands(set(data["lon_group"] for data in grid_data.values()))
    ...
    for lon_band in unique_lon_bands:
        lat_bands_in_lon = sort_lat_lon_bands(
            set(data["lat_group"] for _key, data in grid_data.items() if data["lon_group"] == lon_band)
        )
